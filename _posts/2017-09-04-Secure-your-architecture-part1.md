...
Most of the time this appears in a form sized box where you enter your credentials or a login page via a third party application (Facebook, Google, etc).
...
Commonly used is the 2 factor-authentication, where the client will use a Google Authenticator app to issue a token for your registered application.

## Authorization 
Authorization is the mechanism by which we look at the user's data and see what he is authorized to do.
...
It naturally depends from case to case which applications are in need of your resources. 
Is it a third party application like facebook or a first party like your own application? Or both? 
I will explain both OAuth2 & JSON Web Token and how they answer these questions. 
...
* Zuul authenticates with his app id and secret
...
JSON Web Tokens (pronounced “jot”) are compact, URL friendly and contain a JSON structure.
...
The specifications allows these claims to be customized, allowing additional information to be passed along.
...
For using an UAA server, there are some possibilities to consider:
...
By having multi-frontends in our architecture, the user will have to login to each of these applications.
...
The `@EnableOAuth2Sso` annotation on zuul will forward OAuth2 JWT tokens downstream to the services it is proxying.
...
#### Zuul Filter
Based on Netflix’s Zuul, zuul brings a filter mechanism.
...
Either there is a lack of built-in or easy security controls or we make trade-offs for functionality/features over security.
...
When we added the `@EnableOAuth2SSO` annotation to zuul and `@EnableResourceServer` to our downstream services, we activated the flow to put the token in the header.
...
The problem that occurs is that every microservice would need to connect with the UAA server for verification on every request.
Obviously, we don't want every microservice to be dependant of the UAA server to be available at startup, in testing or CI. 
...
The microservice will handle the token as trustworthy since the token it's verified.
...
#### Authority 
For the authorization, Spring Security provides us with authorities, extracted from the JWT.
The authorities are placed inside a principal, which will be used throughout the existing security context of your application.
