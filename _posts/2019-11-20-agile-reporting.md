...
3 years ago, Ordina proposed me the opportunity to start with a new team for a new project, as team lead.  
...
I went to a talk of [Doc Norton](http://docondev.com/){:target="_blank" rel="noopener noreferrer"} that was called [Escape velocity](https://www.leanpub.com/escapevelocity){:target="_blank" rel="noopener noreferrer"}. The talk was essentially about how and why velocity is incorrectly used as a forecasting and diagnostics tool.
...
The project started as an agile project, so the whole Scrum framework was used. But in hindsight, there were a few problems:
...
We estimated the entire backlog to give the management a rough estimation, 1.5 years of work with 5 developers and 2 analysts.
...
Now, for those reading this and having done such an estimation before, you know that the 1.5 years was probably wrong. And it was.  
After about a year of development, we had to re-estimate the entire backlog which resulted in doubling the amount of story points to 5000. And right now, we're at 6000.  
...
So during the course of the project, I've had to report the progress to the management.  
...
So the end date I had to report to the management would typically move forward each meeting.  
Needless to say, they weren't happy with that. Every single time, I had to explain the fundamentals of Scrum, the backlog, refinements, story splitting, etc …
...
The tool then runs 500 simulations where it randomly picks numbers between the minima and maxima you provided. 
...
Let's take an example. In our backlog, we have 3 features waiting, a total of 150 story points. 
...
During my role as team lead, I've also had to measure the performance and happiness of the team. To do so, it helps to have raw data to analyse. 

## Velocity

The tool we use for managing our scrum process is Jira. It contains various reports that can be generated based on your agile board.  
...
With that spreadsheet, it's just a matter of using the various functions to analyse the data. This gave me results such as the image below.
...
I also asked all team members to enter their hours per sprint in another spreadsheet. Which resulted in this.
...
As always with velocity, it tells us what happened in the past. But what does the chart above tell us?
...
What you do with this data is, as always, up to your team. It can for example help to determine the impact of certain decisions in the past.
...
As velocity, this is a lagging indicator so you can see what happened in the past, but it can't predict the future.

But it's useful to see patterns.  
For example, you can see happiness peaks in sprints where team activities were done. Difficult sprints create happiness lows, due to stress and pressure.
...
If you do it for a longer period, it could like this.
...
How longer we worked together, how more stable the chart gets. Which is a normal evolution as everyone needs to get used to each other in the beginning.

## Sprint Spread

A chart that I started to use the last few months is the spread of the various types of backlog items in sprints.  
...
I find this very interesting to report to the management, as it clearly shows that the team did a lot of work, even if that doesn't translate in raw velocity.
...
And more importantly, it can tell you when there are problems in that process. The image above shows lines that go up evenly, with no drastic changes.

The image below however, shows that the development line is straightening out, which means there is no progress. And thus that will start blocking other teams, such as QA.
...
**As with all things agile, they might or might not work for your situation, but at the least, it might have given some ideas!**
