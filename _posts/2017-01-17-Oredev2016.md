...
so if one container crashes for some reason, the replication controller will look at how much replicas you configured and if one is down, it will schedule a new instance into the cluster.
...
>This can cause some troubles when your process crashes and after obtaining a new IP address.
...
### Services
Is basically a proxy on top of the pod and if you look from a technical perspective, we see configured IP tables.
...
#### HTTP load balancing
Kubernetes does not have support for the following points in production so be sure to have a look at them when deploying to PRD.
* Exposing Kubernetes services to the outside world
* SSL offloading
* Gzip
...
Now, two UX researches at Microsoft deep dived into 100 years of research of signs and symbols and listened to what people define as a good interface.
