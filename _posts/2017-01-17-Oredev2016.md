...
This blog post will be about the talks I favored and where I learned the most from and still use in my nowadays project.

<p style="text-align: center;">
  <img style="max-width:640px;" alt="Reactive Systems Traits" src="/img/oredev/oredevbegin.jpg">
</p>
...
[Adam Bien](https://twitter.com/adambien) is an Expert Group member
 ...
 for Java SE and Java EE projects. He has edited several books about JavaFX, J2EE, and Java EE,
...
He says that involving the container technology to the microservices eco system is considered the future and inevitable.
...
FROM openjdk:8-jre                     // Which base image do I run from?
...
Docker-compose will run all the containers you setup in the configuration.
Since we work with the microservices eco-system, we will need a lot of containers.
...
### Docker-compose
To tell the containers that they have to be under the same network, we will use docker-compose.
...
* networks as property
    * This be the network where you want to be assigned to
* networks
    * This will create one or multiple networks

When you want to add a zuul or an eureka service, follow the steps of `The Microservice` and configure it inside the docker-compose file.
It is important that every container you want to communicate in between stays under the same network.
...
Docker is a part of this world and Emil talks about Docker in a security perspective.

When looking it in a secure way, what are the input sources?
Where does the base image come from?
If we look closer we can see that it retrieves it's images from Docker Hub.
In addition to that, more complexity comes in when we add new packages on top of the base image which will be retrieved from the package manager server.
Finally we have the application package that can come from a CI server and if it is a fat jar, it can consists of different dependencies through maven or some kind.


<p style="text-align: center;">
  <img style="max-width:640px;" alt="hackers perspective" src="/img/oredev/hackers.png">
</p>
...
An attacker could infect the image by his own configuration or modify the orchestration tool.

The most important issue is when the attacker gets control of code execution within our application.
Emil says that it happens quiet often and if it isn't in your code, it is in the third party libraries or the package manager server you're using.

Now you might think that because the attacker is in a docker container that he can't break out.
Think again, he can breakout and he can gain control of the whole operating system.
...
This private registry is insecure by default meaning.
If an attacker gains control of the network where the registry is located, he can easely read and modify each image.
An attacker pushes a modified image and the orchestration will pull this.
...
* Authenticate Users
* Limit access rights
* Docker Content Trust - will digitally sign your images
* Secure you development environment

## How does the attacker inject code?
>You use whatever docker image that is in the registry and add your backdoor binary in a Dockerfile,
...
* Try enable SELinux (enforcing) on the host
...
He's a regular speaker on conferences speaking both about modularity/OSGi related topics, and topics related to Kubernetes and cloud architecture in general.
...
* Run Docker in clusters
    ...
    * Automation - That when a commit ends in a new deployment
...
But for now we see this as one component that has an API.
When a master goes down, the rest keeps running, but in that moment you won't get any scheduling or things like that.

#### Replication Controller
The controller is there to configure where and how much nodes the container has to run in.
It will manage scheduling and monitoring inside the cluster,
so if one container crashes for some reason, the replication controller will look at how much replica's you configured and if one is down, he schedules one more instance into the cluster.
It is important that we don't start our containers ourselves but configure on how many replica's we will need.
After our containers are set up, the cluster will take care of it and monitor it.

### Worker nodes
Here is where your docker containers will run in
#### Pods
...
* Lifecycle of these containers bound together
    * Don't place microservices in to the same pod
* Containers in pod see each other on localhost
* Env vars for services


### Etcd
Is a key-value store and consists of all the configuration of the cluster.
You don't want to lose your etcd otherwise your cluster will go down.
If you want fault tolerance, you will need quiet a lot of machines.
...
>This can cause some troubles when your process crashes and it will obtain a new IP address.
...
If you want the pods to communicate we will be using services as a proxy on top of it.

### Services
Is basically a proxy on top of the pod and if you see at it technically, we see configured IP tables.
When we create a service, it gets a fixed IP address and a DNS name so we can communicate through our services.
Finally it will round-robin traffic to the destined paths and keep track on which of the paths are running.
...
>The front-end talks to the service of the backend pod and this will round robin the call to the right instances.
...
<p style="text-align: center;">
   <img style="max-width:640px;" alt="namespaces" src="/img/oredev/namespaces.png">
 </p>

### Kubernetes in Production

#### HTTP load balancing
Kubernetes does not have support for these in production so be sure to have a look at it when deploying to PRD.
...
#### NodePort
* Expose service ports as ports on physical nodes
...
She is equally happy debugging nasty back-end issues as she is developing apps or working on UI/UX design.
She has been blogging at [her blogsite](http://blogs.msdn.microsoft.com/Tess) for the better part of her career, and spoken at lots and lots of conferences around the world.

Software is build in to two parts, it is what we see and how we do it.
In this session Tess talks about what these features should look like.
If you design 'an order' incorrectly and the user can't figure out to place that order, you have a bad UX design and it will cost your company a lot of money.
Now two UX researches at Microsoft deep dived into 100 years of research of signs and symbols and listen to what people find a good interface.
At last they categorized every piece in to 'Tenets' (Attributes of good user interfaces) and 'Traps' (Bloopers and things to avoid)
...
If you look at the scanner in a supermarket, do you know which button you have to press to save a product?
...
----------------------
>Is it what I am holding quick and comfortable to use?
When scanning a product you want your users to feel that they can scan the product easily and with no difficulty.


Responsive
----------------------
> You get immediate feedback, when scanning a product you get an immediate response ( ex: Hey! you scanned something )
If you don't get an immediate response, the user will keep trying to scan the product and at the end have 10x milk boxes.

Efficient
----------------------
> What is the most efficient way in a process.
The best way to buy 10 milk boxes is to scan the milk 10 times than to write down 10x with your other hand.

Forgiving
----------------------
> I can undo my actions.
If I scan too many items, I have a minus button to lessen the amount.
It's a bad practice if there is no button and you have to ask an employee to help you.

Discrete
----------------------
> Don’t over share, don’t overreact.
A siren on the scanner to point out that you did something wrong.

Protective
----------------------
> Always deliver a qualified product with no failure or data loss
A bad practice is if you fill in a form, go to the next page and realising that you forgot something on the previous page and that your form is blank
...
Never Change your structure too many times and if you update, be sure that you remind your users of the change ahead.

Beautiful
----------------------
> I find it attractive compelling.
Make the user think you put a lot of effort in it
Use same buttons, nicely aligned and use the same colors

## Traps: attributes to avoid
Every 'Tenet' has his own set of traps, each trap is there to learn us to avoid it as much as possible.
...
##### ***Invisible Element***
The user has learned a critical hidden capability but cannot see it.
...
Accidental activation if shut down

#### Responsive Traps
* Be sure your response is half a second and not greater.
* If the response is greater show a loading circle or show the log of the process

#### Efficiency Traps
##### ***Information overload***
Have a page that shows you too much information, be sure to keep it as simple as possible.
##### ***System amnesia***
When they show you information that you don't need
##### ***Autocorrect***
The name says it all, when autocorrect changed your word in something you don't need.

#### Habituating Traps
* Dong things so many times it gets automatic and then it changes.
...
* Walk through all the ways the user could complete each task
* Identify and log any traps you observe
* If you cant find a trap, identify the tenet
...
<img class="float-image p-image" src="{{ '/img/oredev/sebastien.jpg' | prepend: site.baseurl }}" alt="Adam Bien" style="width: 100px"/>
...
When talking about creating our own protocol you know that it is recommend not to invent the third wheel and use HTTP instead.
...
* Typed links - A link that know where to go next
* Reuse existing libraries (HAL)

### Common Language
JSON-LD will try to use a common language for everyone to understand.
...
Finally the exact uri link will be http://schema.org/Person
If you go to this uri, you'll get this:


<p style="text-align: center;">
   <img style="max-width:640px;" alt="person resource" src="/img/oredev/personjson.png">
 </p>
...
## Conclusion
Øredev is one of the best conferences I attended, not only did it innovate in different talks, it managed to organize so many topics to go to.
