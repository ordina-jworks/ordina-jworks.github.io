...
Then **Continuous Integration** had been set up in order to prevent the merging issues.
...
Developers start with the assumption that it is worthy, CI tool decides if it is.
Deploy frequently with small changes, makes it easier to rollback and lowers the risk of running into problems.
