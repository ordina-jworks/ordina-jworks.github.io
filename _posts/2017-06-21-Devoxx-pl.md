...
In Git branches where much more lightweight and every repository contains the full source history.
...
With tools like [split.io](https://www.split.io) and [launch darkly](https://launchdarkly.com) GitFlow is not needed, if merged frequently.

To conclude, Sam challenged the audience and asked if branches are evil.
No, experimental and release branches that might never get merged, have their uses.
