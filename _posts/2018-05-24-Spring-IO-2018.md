...
First he described us Kafka, a very potent messaging system which allows you very easily to act as a throughput between your applications, just make sure to stay away from [recreating ESB antipatterns with Kafka](https://www.thoughtworks.com/radar/techniques/recreating-esb-antipatterns-with-kafka).
...
Of which the results can very easily be pushed to external systems (HDFS, S3, JDBC) or consumed by many clients.

To be scaleable, Kafka is kept simple at the core, all data is stored as a partitioned log. 
Meaning that writes are append only and reads a single seek & scan.
The underlying filesystem can thus very easily handle the storing & caching of messages, also when reading data is copied directly over from the disk buffer into the network buffer bypassing the JVM.
Ideally for flooding your network.
