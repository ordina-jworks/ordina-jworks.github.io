...
### Pipeline
Amazon's pipeline service is called [AWS CodePipeline](https://aws.amazon.com/codepipeline/){:target="_blank" rel="noopener noreferrer"}. This service allows you to create a pipeline which executes all the necessary steps to automate your releases and to provide infrastructure updates. This allows you to quickly provide new releases and deliver new features to your end users. You can integrate this service with other external services (ex. Github, where your repository is hosted). 

To effectively build your application, you can make use of [AWS CodeBuild](https://aws.amazon.com/codebuild/){:target="_blank" rel="noopener noreferrer"}. This is their continuous integration service that packages your application by building it and running the tests. Pricing is very effective, as you will only pay by the minute that you trigger a build. CodeBuild automatically scales and can process multiple builds at the same time, so you won't have to deal with build queues. 
...
One of the most impressive features is the Blue/Green deployment. This splits the traffic going to your application into traffic to your ortiginal environment and an other environment. The best use case for this would be during a new production release. You have a new release that you want to deploy? Simply let 10% of your traffic go to that release to make sure everything works right. After monitoring the release, you can choose to either revert or to fully allow traffic to go to the newest release.

### Conclusion
Amazon provides a lot of tools that help a project to manage their builds and deploys, making sure that everything works according to plan. We were definitely a fan of their new Cloud Development Kit and their Blue/Green deployment is able to stabilize and fully control the rollout of a new deployment.

# Integrate your front end apps with serverless backend in the cloud by Sebastian Stormacq
Serverless is a big trend nowadays and rightfully so, as it does a lot of stuff for you. The biggest pros of serverless development are that you no longer need to provision or manage your own server, it automatically scales with the amount of usage, it has built in fault tolerance and fallback methods and last but not least, you only pay for the value. This means that if you are not using your lambda, you are not paying for it!

Sebastian filled the session mostly with a demo where he prepared a React app to communicate with his lambda's. 

To provide his infrastructure (auth, GraphQL API etc.), he used the [AWS Amplify CLI](https://github.com/aws-amplify/amplify-cli){:target="_blank" rel="noopener noreferrer"} which is a new CLI tool which can add cloud features to your web or mobile application. For example, by simply typing `amplify add auth` and `amplify push`, you have created your own user authentication provider (Cognito). And if you want a GraphQL API (AWS AppSync) set up, you just have to type `amplify add api` followed by `amplify push`. This creates an API and stores its data on a DynamoDB database.

To finally deploy his application, he simply used an S3 bucket that acts as a web server. After all, React just builds static HTML / CSS / JS files which need a web server (Nginx, Apache) in order to be displayed. Using an S3 is very cost effective and doesn't require anything else other than a bucket. So no linux configuration, load balancer setup & costs, ... . By using `amplify add hosting` and `amplify publish`, these steps are automatically done and doesn't require us to do anything else. You can also enable production mode, which enables AWS CloudFront integration to send your website to edge locations, so you can provide faster loading times to your users from all over the world. 
...
The rest of the session was a very interesting take on how you actually split up your microservice. It's useful to have multiple codebases per service with their own deploy pipeline to deploy to your environments. 

It's also a good practice to isolate your application from your dependencies. You also should explicitly say which dependencies you want and which version that you want. This helps you to build and create immutable images with explicitly defined dependencies, so you can run the same image in multiple environments.
