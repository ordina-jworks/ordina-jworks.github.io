...
* [Kubernetes and/or Cloud Foundry - How to run your Spring Boot Microservices on state-of-the-art cloud platforms by Matthias Haeussler](#kubernetes-andor-cloud-foundry---how-to-run-your-spring-boot-microservices-on-state-of-the-art-cloud-platforms-by-matthias-haeusslerhttpstwittercommaeddestarget_blank-relnoopener-noreferrer)
...
When starting with measurements an certain app started up in 1300 milliseconds.
That went down to 1200 milliseconds by playing with Spring.
...
If the `Classloader` has been warmed up, startup is even much much faster.
With Spring Devtools you have a warm `Classloader`, which reduces your startup time.

Lots of optimizations have happened eg. heap memory went from 10 MB to 6 MB with the move to Spring Boot 2.
...
It shows the calls being executed
The width of the flame is the time it took to run it.
...
* `spring-context-indexer`: this is marginal.
 With a lot of beans it will have a bigger impact.
* Spring actuators used to be costly for start up, but has now it no longer has a big impact.
...
It does make sense to do when developing in order to improve development time.
* Unpacking the fat jar: running with explicit classpath `java --jar` is little bit slower than running with `java --cp`.
...
* Lazy beans: Pay attention to custom beans with expensive `@PostConstruct`.
...
You can implement an `ApplicationContextInitializer` which makes you reflection free, but is harder to implement.
...
However, it's not exactly the same as working with a real PostgreSQL server.
 ...
 I found this part very interesting, as I had an issue exactly like this a couple of weeks ago.
...
You use querries for reading the data and commands for updating the data.
While for basic CRUD operations having these models combined might be fine, once the amount of business logic and amount of querries increases it might become increasingly difficult to manage.
...
It comes with a preferred stack on which I will dive into more detail later.
You can also get away from this and use your own prefered services albeit with some additional configuration.
...
It uses JPA for storing data and Infinispan for the horizontal scaling of multiple Keycloack nodes that all distribute information like user sessions between eachother.
...
You'll also have to register your application within a Keycloak realm.
Once you have done this, after doing some configuration within your application and the Keycloak management console,
...
* Application receives code associated with sign-on session and sends the code back to Keycloak via a separate channel
...
Together with a more in depth explanation of the covered topics in a video of the talk below.
