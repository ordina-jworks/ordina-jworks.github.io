...
Nicolas Frankel is a Developer Advocate who works for Exoscale, which is an European cloud hosting provider.
...
Docker images are and should always be immutable. They should be configurable depending on the environment where we want to run our image in.
A Docker image should be able to run in different environments without problems. This is where Kubernetes comes in. 
Kubernetes can easily configure and parameterize each Docker image to run in different environments.

One thing to remember is that you make sure you are working in the correct environment. Nicolas likes to add banners to the page
to know in which environment you are currently working. For example, if you are working in the development environment, then you might
want to show a big blue 'Development' banner.


This can all be done with the power of Kubernetes and immutable images. You can simply declare your environment variables in Kubernetes,
then you can inject your environment in your Spring Boot application.
...
Ofcourse, you can already do this with [Spring Cloud Config](https://spring.io/projects/spring-cloud-config). This is just an alternative on the Kubernetes side of configuration management.
...
We are currently considering merging multiple of them into a more coarser grained architecture containing multiple more 'monolithic' applications.
...
The key take away here is that although there’s an advantage in terms of architecture degradation (it’s harder to accidentally call another microservice than to call a method on another bounded context in a monolith),
this advantage comes at a cost. You lose compile-time safety making it harder to refactor than a Monolith and it’s harder to test the whole system.
These are especially disadvantageous in the early stages of the project when it’s not clear yet what the correct bounded contexts are.
...
There are ofcourse other ways to divide your application into Modules and maintain the architecture: 
...
and it can become kinda verbose with all the configuration(pom/build.gradle) files.
...
It alleviates some of the biggest problems with monoliths without introducing new ones using a complexer architecture like microservices!

## Cutting-Edge Continuous Delivery with Spinnaker by [Andreas Evers](https://twitter.com/andreasevers){:target="_blank" rel="noopener noreferrer"}
...
You can keep still configure this as code:
```java
...
We will then back off and stops flooding it with requests
we can save system resources for calls which are likely to fail.
And give the other service some time to get back on its feed.
...
The Axon framework is used for building event-driven microservices using Domain-Driven Design, CQRS and Event Sourcing.
...
It is easy to go through the history and generate the current state while also getting a lot of extra information about the object you would otherwise miss out on when just storing it's current state. 
...
They need to be the source of everything in the application and how a true representation of your entities.
Once again, you dont save the state of an aggregate, you can generate the state by replaying the history.
...
### Axon Server
Axon Server is a service that distributes your components, manages routing, stores events and provides high availability and observability.
By combining all these otherwise different services into one single easy to configure service,
you make your entire architecture a lot easier to manage than if you were to use for example the Netflix Eureka Discovery Service for commincation between microservices,
