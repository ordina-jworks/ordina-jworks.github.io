...
An API management tool helps you with setting the responsibilities on the right level for your exposed services, and so ensure that your services implementing all required policies and security.  
...
An API gateway is actually a middleware that is placed in front of your services an act as a single entry point to administrate, route and secure your services.
...
This sometimes leads to duplicate code and the fact that your API is not only responsible for exposing the Data, but also for implementing other cross cutting functionalities.
