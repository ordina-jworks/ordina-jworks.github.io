...
* Second removed the java ee and corba modules, together with some other apis resulting in 19 modules
...
When building out search functionality it is important to know how do your users search.
Do the use one word, two words, three words, …
User queries should be saved, don’t forget to do so, so you can learn more about your users and improve your searching.

Some quick core concepts:
- TD/IDF: rare words score higher
- Edit distance: minimum number of operations required to transform one string into another: ex. star -> start
...
Where he showed how "good" his initial search was: `star war` vs `star wars` where the first one just showed some zombie movies but nothing over star wars.

During the live coding session several search implementations were made to see the difference: match on title alone, match on title, description, etc

Even after some modifications we still have the sitation when searching on `star war` that both `star` and `war` are matched separately.
...
- For tests, the Cranfield Evaluation Method is a good strategy to implement tests: for every change you do in your model you need to verify if your query still returns a good result. It is more complicated as the average junit test though.
