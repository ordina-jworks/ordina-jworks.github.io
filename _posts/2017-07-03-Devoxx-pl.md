...
Previous Garbage Collectors were not so scalable nor predictable.
The G1GC promises a more scalable and more predictable system with few modification options.

By default one fourth of the physical ram will be allocated to the heap, unless the size is specified with the `-mx` flag.
...
Oleg concluded by recommending the audience not to touch multi-release jars, jlink and Unsafe, unless you are 100% sure what you are doing.
...
Vaughn started his talk by introducing Rear Admiral Grace Hopper to the audience.
...
Another quote was shown us from Donald Knuth:
