...
[Slides](https://speakerdeck.com/shelajev/a-reasonable-overview-of-java-9-devoxx-pl-2017)
...
A new `ProcessHandle` will be able to get information and control processes.
...
Property files will support UTF 9 and there is already Java 9 support in several IDEs.
There will be several changes to improve `String` performance, by for example using a more space-efficient internal representation for Strings.
...
A module can define dependent modules with _requires_, to provide an API the _exports_ key word is used.
To give access to everyone the _opens_ keyword can be used.

There is a method `getAccessable`, beware using it for determining if a module is usable.
...
Otherwise you might need to modularize twice to align with dependencies.
...
Previous Garbage Collector's were not so scalable nor predictable.
The G1GC promises a more scalable and more predictable system with few modification options.

By default 1/4 of the physical ram will be allocated to the heap, unless the size is specified with the `-mx` flag.
...
Oleg concluded with recommending the audience not to touch multi-release jars, jlink and Unsafe, unless you are 100% sure what you are doing.
...
## The Language of Actors. ([Vaughn Vernon](https://twitter.com/@VaughnVernon))

[Slides](https://github.com/VaughnVernon/DDDwithActors)

Vaughn started his talk with introducing Rear Admiral Grace Hopper to the audience.
In the American Navy she was a Computer Scientist and wrote software for a long time.
She was much into not wasting cycles and emphasised on not wasting nanoseconds.

Then Vaughn introduced Donald Knuth, another legend in Computer Science.
Knuth is known from the quote _Premature optimization the root of all evil_.
But that is not exactly what he said, the full quote says:
_We should forget about small efficiencies, say about 97% of the time: premature optimization is the root of all evil. 
Yet we should not pass up our opportunities in that critical 3%._
Another quote was shown us from Donal Knuth:
_People who are more than casually interested in computers should have at least some idea of what the underlying hardware is like. 
Otherwise the programs they write will be pretty weird._
...
To improve the usability of the application it was rewritten in the language C, allowing the software to fit on just one diskette.
With this introduction Vaughn wanted to emphasize how hard it is to optimize software for resource usage.

### Threading is hard

In 1973 academics discovered the **Actor Model**, 13 year later in 1986 Joe Armstrong rediscovered the approach.
Armstrong designed and implemented a programming language on this model, **Erlang**
Jonas Bonér 2008 came up with Akka for the Java Virtual Machine, in 2011 José Valim came up with another Actor based language Elixir.
...
The actor model allow us to embrace latency. If we design for latency, it will not have a blocking impact on the design.
