...
This talk is ideal for people wanting to get a brief bottom-up overview of how SSL and TLS works.

### Defence Against the Docker Arts

Joe Kutner works at Heroku and core maintainer of [Buildpacks.io](https://buildpacks.io/){:target="_blank" rel="noopener noreferrer"}

He starts this talk by providing a set of common pitfalls and best practises for creating Dockerfiles in rapidfire.
Multi-stage builds, combining multiple run commands into one and using proper base images as a base for all docker builds are a few examples.
...
Next, Joe explains what Buildpacks are, their origin, their usecases and what the architecture is.
...
Joe demos the usefulness of Buildpacks with the usecase of the required HeartBlead CVE.
...
They introduce the different standards that Flowable supports.
Tijs continious by explaining the architecture of Flowable and it's evolution.

Next, Joram explains how events are used in the Engine.
He continious the talk by providing a simple demo of the platform using an example process.
...
Container security, monitoring and logging and their solutions for these requirements are shown and explained.

This is an amazing talk, a definately must watch for anyone using Kubernetes in a regulated environment.

Questions

* Why not use Docker registries of the specific clouds?
* What underlaying networking CNI in Azure?
* CI/CD mentions Azure DevOps, vendor locking, why?
* Was Kustomize considered?
* Vault integration? Kube sync or directly in service?
...
During the conference a lot of tools (especially Jib popped up a few times) and integrations with IDEs have been showcased and every talk just assumes that the deployment platform is Kubernetes.
