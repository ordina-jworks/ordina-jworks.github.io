...
### The backend itself
Since we are called JWorks and we mainly focus on Java/Javacript development it should not come as a surprise that our backend application is written in Java with the Spring Boot framework.  
In general we prefer to write backends in the microservices paradigm, but in this case the backend was sufficiently small that it only consists of a single microservice.  
...
Our frontend application consists of two parts. The first one is aimed at citizens who wish to help the cause, who can notify this surveillance network when they find a bottle as shown in the image below. The second part is aimed at the researchers, and could be seen as the “backend” of the project, where the data given by the GPS trackers and the citizens is visualized in a clear and orderly way.
...
The sensors can receive instructions and updates, but this has to happen inside of the same open tcp connection within a very short timeframe.
This is why we have chosen to develop a separate java sensor gateway to handle these incoming messages. This is deployed on an elastic beanstalk.
