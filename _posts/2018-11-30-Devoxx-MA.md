...
* [MockK, the idiomatic mocking framework for Kotlin by Yannick De Turck](#mockk-the-idiomatic-mocking-framework-for-kotlin-yannick-de-turck)
...
<span class="image left"><img class="p-image" alt="Yannick De Turck" src="/img/devoxx-ma-2018/speaker-tom-van-den-bulck.jpg"></span>
<span class="image left"><img class="p-image" alt="Yannick De Turck" src="/img/devoxx-ma-2018/speaker-tim-ysewyn.jpg"></span>

Our colleagues, Tim Ysewyn and Tom Van den Bulck, gave a talk about Stream Processing Live Traffic Data with Kafka Streams.

Tom did the theoretical part of the talk, he started with the bigger picture. What is Stream Processing, computing on data directly as it is produced or received.
...
The image above shows us the time before stream processing. At that time the data was mostly stored in databases and file storage when it became available. Applications would process the data afterwards.
...
With stream processing the data will be processed directly and recreate a new event for other applications if needed. The applications will react on an event instead of scheduling jobs to file storages.

Tom did a great job by explaining this all to us, they both explained the data we were going to use. The Flemish government holds an XML document with Flemish words for explaining the Live Traffic data. This document is recreated each minute so we can create a scheduled job to create an event out of it. This was already prepared for us during the timeslot.

Because a theoretical point of view is some difficult to follow, Tim has showed during live coding who we can handle the events created and do some crazy stuff.

We learned how we can use Spring to help us with Kafka, how we can get the data each minute with code and with annotations. Who we can handle those events and do something with it, we created an average speed for a specific sensor during a timeframe of 2 minutes.

[Slides](https://www.slideshare.net/TimYsewyn/stream-processing-live-traffic-data-with-kafka-streams){:target="_blank" rel="noopener noreferrer"} || 
[Demo](https://github.com/Stream-Processing-with-Kafka-Streams/workshop){:target="_blank" rel="noopener noreferrer"}
...
<span class="image left"><img class="p-image" alt="Saskia and Lois" src="/img/devoxx-ma-2018/speaker-saskia-lois.jpg"></span>
Some background information, Saskia (13y) and Lois (10y) Blanc, they asked the audience to close their eyes for a few seconds. 
We must think about tomorrow, what would we like to have in the near future.

The asked the audience to explain what we’re thinking, some basic answer like, self-driving-cars, robots to help in the household, …. 
They started off to explain how they see the world, but that was aligned with ours. Saskia started with some explanation about Logo (programming language) while Lois was performing the coding. Lois asked his big sister if there was an easier way to create a method/procedure to create a square.

Lois on the other hand explained Scratch, it’s a program where you can create your own games, animations and interactive stories. 
He also showed the community around the program. 
The ease of drag and drop was amazing to see.

Last but not least Saskia programmed live on stage in Groovy. 
She asked her dad what he did, and she wanted to learn to code, her father is a Java programmer, so he linked directly to Groovy. 
He explained her some basics and she created a Animal interface with a Cat class that meowed.

Both of them wanted to become developers of the new generation.
