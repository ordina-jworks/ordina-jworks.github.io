...
[Destination rules](https://istio.io/docs/concepts/traffic-management/#destination-rules){:target="_blank" rel="noopener noreferrer"} are a set of rules that are executed when a services is called.
...
This demo assumes that Istio is already install in the cluster with the demo profile enabled. 
See [Install Istio](https://istio.io/docs/setup/kubernetes/install/helm/#option-1-install-with-helm-via-helm-template){:target="_blank" rel="noopener noreferrer"}
In the repository, a small [script](https://github.com/pietervincken/istio-service-to-service-demo/blob/master/1-install-istio.sh){:target="_blank" rel="noopener noreferrer"} can be found that can assist in setting up the demo environment

### Setup

The basic setup of our application is a very simple service with a database backend. 
Our service exposes one HTTP GET endpoint which will be accessed by the outside world. 
Our database is a Apache Couchdb instance. 
...
This looks like a normal deployment of a Couchdb database.
...
Instead of using this approach, Istio now has the option to rewrite the probes during the automatic proxy injection. 
...
It is a namespace scoped policy telling Istio that all services should **ONLY** accept mTLS connections.
...
When a request would be send to the test-app service now, all requests would be rejected with an HTTP 503 error code.
...
This limits the reach a single services has in the cluster and therefor adheres to the least privileges principle.

The following manifest defines an cluste RBAC configuration.
...
Specifying the namespace with-istio in the inclusion section, Istio enabled RBAC for all services in that namespace.
...
This manifest creates a role that allows the couchdb service to accept GET requests for any given path. 
Note that the full service name is used in the services specification, this is currently required by Istio.
This is only needed for Istio to identify the traffic, services can still specify the short name (namespace scoped).
...
When roles are connected to the service account of a service, the service can access the services specified in the role.
