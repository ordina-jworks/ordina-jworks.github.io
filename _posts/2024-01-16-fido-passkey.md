...
It appears to be high time to address the vulnerabilities associated with traditional username/password logins and transition towards the use of FIDO.
...
* Strong Security: FIDO Authentication utilizes public-key cryptography, which is more secure than traditional passwords against phishing attacks, keyloggers, and other threats.
...
So, how is this technology employed in FIDO? During registration, your device is generating unique passkey witch contains public key and private key.
...
To authenticate a user, we send our username to the server, the server will answer with a challenge encrypted with our public key.
...
After sending the encrypted response, the server is able to validate the response with our public key and check if the login succeed.
...
We need to save following data for the user.
...
We need to save following data for the passkey.
...
### Create services
We are going to create 2 services (`RegisterService` and `LoginService`)
